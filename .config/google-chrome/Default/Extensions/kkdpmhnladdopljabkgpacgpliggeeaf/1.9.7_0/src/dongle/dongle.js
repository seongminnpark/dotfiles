        ...
        return function(result) {
          ...
          task = function() {
            ...
            if (ledger.config.network.ticker === 'abc') {
              promise = _this._btchip.createPaymentTransactionNewBIP143_async(false, inputs, associatedKeysets, changePath, outputScript, lockTime && new ByteString(Convert.toHexInt(lockTime), HEX), sighashType && new ByteString(Convert.toHexInt(sighashType), HEX), authorization && new ByteString(authorization, HEX), resumeData);
            } else {
              if (ledger.config.network.handleSegwit) {
                promise = _this._btchip.createPaymentTransactionNewBIP143_async(true, inputs, associatedKeysets, changePath, outputScript, lockTime && new ByteString(Convert.toHexInt(lockTime), HEX), sighashType && new ByteString(Convert.toHexInt(sighashType), HEX), authorization && new ByteString(authorization, HEX), resumeData);
              } else {
