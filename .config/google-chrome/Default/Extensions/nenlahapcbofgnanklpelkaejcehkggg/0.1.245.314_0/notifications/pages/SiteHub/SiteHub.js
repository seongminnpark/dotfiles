...
function CloseIcon(props) {
  return <div className="icon-x" {...props}></div>;
}

function Logo(props) {
  return (
    <div className="w-icon-logo" {...props}>
...
class SiteHub extends Component {
  ...
  render() {
    ...
    return (
      <div className={`wb-SiteHub ${visible ? 'visible' : ''}`}>
        <div className="header">
          <a href={WIKIBUY_URL} target="_blank"><Logo/></a>
          <Gear
            onClick={this.onSettings}
            style={{top: '12px', right: '50px'}}/>
          <CloseIcon onClick={this.onClosePopup.bind(this)}/>
        </div>
        <div className="container" ref={(n) => this.scrollContainer = n} style={noMinHeight ? {minHeight: '0px'} : null}>
          {this.props.children}
        </div>
      </div>
    );
