...
const CompareItem = ({className, result, hasReward}) => {
  ...
  const deliveryInDays = minDays && maxDays && minDays !== maxDays ? `${minDays} - ${maxDays}` : minDays || maxDays ? `${minDays || maxDays}` : null;
    ...
    <div className={`${className} compare-item`}>
      ...
      <div className="summary">
        <div>
          ...
          {
            deliveryInDays ?
            <h6>Arrives in {deliveryInDays === '1' ? `${deliveryInDays} day` : `${deliveryInDays} days`}</h6> :
            shippingSpeed ?
            <h6>{shippingSpeed} Shipping</h6> : null
          }
        </div>
      </div>
    </div>
...
class SavingsDetails extends Component {
  ...
  render() {
    ...
    const reward = _.get(result, 'pricing.reward.amount');
    const percentOff = _.get(originResult, 'pricing.total') ? savings / _.get(originResult, 'pricing.total') : 0;
    const percentOffPercent = numeral(percentOff).format('0,0%');
    return (
      <AnnotationTooltip onCloseTooltip={this.props.onCloseTooltip} className="savings-details-tooltip">
        {
          resultIsAmazonMarketplace ?
          <h2>Save <span className="bold">{formatCurrency(_.get(result, 'savings'))}</span> buying from a different <br/>Amazon seller.</h2> :
          savings ?
          <h2>Save <span className="bold">{formatCurrency(_.get(result, 'savings'))}</span> buying from a different seller.</h2> :
          reward ?
          <h2>Get <span className="bold">{reward/100}%</span> back buying from a different seller.</h2> : null
        }
        <div className="comparison" onClick={() => this.props.viewProductPage()}>
          <CompareItem className="origin-result" result={originResult} hasReward={!!reward}/>
          <CompareItem className="wikibuy-result" result={result} hasReward={!!reward}/>
        </div>
        {
          resultIsAmazonMarketplace ?
          <button className="button-style primary-btn-large" onClick={this.props.addToAmazonCart.bind(this, result)}>
            Add item to Amazon Cart {percentOff && percentOffPercent !== '0%' ? <span><span className="pipe">|</span>{`${numeral(percentOff).format('0,0%')}`} off</span> : null}
          </button> :
          <button className="button-style primary-btn-large" onClick={() => this.props.viewProductPage()}>
            Continue to Wikibuy {percentOff && percentOffPercent !== '0%' ? <span><span className="pipe">|</span>{`${numeral(percentOff).format('0,0%')}`} off</span> : null}
          </button>
        }
      </AnnotationTooltip>
    );
