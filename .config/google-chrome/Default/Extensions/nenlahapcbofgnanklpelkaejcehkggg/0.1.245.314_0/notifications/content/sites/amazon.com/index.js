...
function runASIN(asin, price, shipping, seller, atcData, product, feedback, dealId) {
  ...
  checkPageData(window.location.href, {type: 'run', asin, price, shipping, seller, product}).then((data) => {
    if (data) {
...
function getProductResults() {
  ...
  _.forEach(els, el => {
    const source = el.querySelector('.a-carousel-heading').innerText.replace('(What\'s this?)', '').trim().toLowerCase();
...
function extractFeedbackData(price, asin) {
  try {
    ...
    if (!price) {
      const priceEl = document.querySelector('#moreBuyingChoices_feature_div span.a-size-small > a');
      if (priceEl) {
    ...
    const rankings = _(els)
      ...
      .filter(r => r && r.url && r.url.indexOf('bestsellers') !== -1 && r.url.indexOf('_last') !== -1)
      ...
      productInfo = _(infoString.split('\n'))
        .map(s => {
          ...
          if (components.length >= 2) {
            ...
            const value = components.splice(1).join(' ').trim();
            if (value.match(/((january|february|march|april|may|june|august|september|october|november|december)\s[0-9]+,\s[0-9]+)/i)) {
              introduced = value;
            }
            return {name, value};
          }
...
function getMainImage(asin) {
  ...
  const selectors = ['#landingImage', '#imgTagWrapperId img', '.image img', '#imageBlock [data-a-dynamic-image]'];
  _.forEach(selectors, s => {
    if (image) {
      return;
    }
...
function checkGlobablIgnoreList() {
  ...
  if (atcEl) {
    ...
    if (text && text.match(/amazon digital services llc|pre-order: add to cart|add to rental|buy and get code/i)) {
      return true;
    }
  }
  ...
  if (titleText && titleText.match(/gift\s?card|\[digital\scode\]|key\scard|pc\sdownload|protection\splan/i)) {
    return true;
  }
  ...
  if (titleText && titleText.match(/\bbundle/i) && !titleText.match(bundleExceptionPattern) && !subNavText.match(/video\sgames/i)) {
    return true;
  }
...
async function asinUpdateCheck() {
  ...
  let updateCount = 0;
  while ((!document.getElementById('ASIN') || !document.getElementById('title')) && updateCount < 100) {
    await delay(50);
    ++updateCount;
  }
  function onChange() {
