...
export async function auth() {
  try {
    ...
    const {url} = await xhr('GET', `${ACCOUNT_SERVICE_URL}/third_party/google?redirectUrl=https://wikibuy.com/oauth/google`, null, null, null, decodeURIComponent(s));
    return url;
  } catch(err) {
...
export async function getContacts(importFlow) {
  ...
  const s = tree.get('sessionToken');
  try {
    const contacts = await xhr('GET', `https://api.wikibuy.com/v1/google/contacts`, null, null, null, decodeURIComponent(s));
...
export async function authWithGoogle() {
  ...
  openedWindow = window.open(url+'&popup=true', 'Wikibuy', 'top=' + winTop + ',left=' + winLeft + ',toolbar=0,status=0,width=' + winWidth + ',height=' + winHeight);
  try {
