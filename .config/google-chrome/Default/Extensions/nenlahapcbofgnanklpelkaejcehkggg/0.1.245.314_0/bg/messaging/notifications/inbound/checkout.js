...
export default async (request, tab) => {
  if (_.get(request, 'paymentsSupport')) {
    chrome.tabs.create({url: `${WIKIBUY_URL}/run/${request.runId}/place-order/${request.detail.id}${request.paypal ? '/?paypal=true' : ''}`, active: true}, (tab) => {});
  } else {
    if (hasFeature('cb_vm_activate')) {
      const cashback = await getCashback({url: `http://${_.get(request, 'detail.product.vendor')}`});
...
// Add to amazon cart
// if (_.get(request, 'detail.product.vendor') === 'amazon.com') {
//   await addRunResultToCart(_.get(request, 'detail'));
