...
import {WIKIBUY_API, WIKIBUY_URL, ORIGIN_SITE_API} from 'constants';
...
chrome.webRequest.onBeforeSendHeaders.addListener((details) => {
  if ((details.url.indexOf(WIKIBUY_URL) > -1 || details.url.indexOf(WIKIBUY_API) > -1 || details.url.indexOf(ORIGIN_SITE_API) > -1)) {
    details.requestHeaders.push({
      name: 'x-wb-extension',
      value: 'true'
    });
  ...
  return {requestHeaders: details.requestHeaders};
}, {urls: ['*://*.wikibuy.com/*', '*://*.ivf-dev.com/*', '*://*.ivf-local.com/*', '*://*.ivaws.com/*'], types: ['xmlhttprequest', 'main_frame']}, ['blocking', 'requestHeaders']);