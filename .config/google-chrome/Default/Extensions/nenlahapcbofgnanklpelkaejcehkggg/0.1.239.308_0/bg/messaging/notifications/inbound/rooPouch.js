...
export default (data, tab) => {
  try {
    ...
    if (action !== 'error' && runs[tab.id] && new Date().getTime() > runs[tab.id].startTime + SOFT_RUN_TTL) {
    ...
    if (action === 'pageLoad') {
      ...
      for ui purposes */
      return getNextCode(runs[tab.id]) || (beforeCodeSavings(runs[tab.id]) && _.last(runs[tab.id].coupons).code);
    }
  } catch (e) {
    return {
...
function beforeFinalTotal(run) {
  return run.results.length === run.coupons.length &&
    !_.isUndefined(_.last(run.results).savings) &&
...
function saveTotal(tab, total) {
  ...
  if (beforeInitialTotal(run)) {
    ...
    runs[tab.id].results[lastIndex].savings = runs[tab.id].originalTotal - total;
  }
}
