...
class AffiliateOffers extends React.Component {
  ...
  render() {
    ...
    if (!this.props.totalsLayout || (this.props.cartPricing && this.props.cartPricing.paymentsSupport)) {
      offers.shift();
    }
    ...
    return (
      <div className={`affiliate-offers ${this.props.totalsLayout ? 'totals-layout' : ''}`} onClick={(e) => e.stopPropagation()}>
        {
          _.map(!this.props.limitResults ? offers : this.state.showAll ? offers : _.take(offers, NUMBER_TO_SHOW), (offer, i) => {
            const productFeedback = _.find(_.get(this.props.feedbackData, 'products'), {id: `product.${_.get(offer, 'product.wbpid')}`});
            return (
                      ...
                      <h5 className="bold" onClick={this.onClickShowMoreEbay.bind(this)}>
                        {
                          ...
                          <span className="show-more"><span>fewer eBay offers</span> <span className="disclosure less"></span></span> :
                          <span className="show-more"><span>more eBay offers</span> <span className="disclosure more"></span></span>
                        }
                      </h5>
  ...
  onClickShowMoreEbay() {
    sendMetric('trackClick', 'showMoreEbayOffers', this.state.showMoreEbay ? 'less eBay offers' : 'more eBay offers');
