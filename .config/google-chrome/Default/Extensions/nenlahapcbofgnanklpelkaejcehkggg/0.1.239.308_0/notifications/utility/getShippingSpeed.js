...
function orderStatusMap(status, shippingKey, arrival) {
  ...
  if (shippingStatus) {
    if (status === 'delivered') {
      ...
      shippingStatus.shipping = `arrives in ${_.get(getShippingMeta(shippingKey), 'description', 'about a week')}`;
    }
  }
...
export function getInstantOrderStatus(statusKey, arrival, sent) {
  ...
  if (status === 'canceled' || status === 'delivered' ||  status === 'return_pending' ||  status === 'refunded' ||  status === 'returned' ||  status === 'closed') {
    return {
      status,
      hasItem: true,
      statusKey: statusKey
