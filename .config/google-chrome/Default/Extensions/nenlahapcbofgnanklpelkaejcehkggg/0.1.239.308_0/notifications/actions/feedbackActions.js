...
function setUpdatedFeedback({feedback, updatedFeedback, type, data}) {
  if (data.parent_id) {
    const parentCursor = tree.select(['ProductPage', 'feedbackData', getTypePlural(type), {id: data.parent_id}]);
    parentCursor.set(['comments', {id: data.id}], updatedFeedback);
    if (data.parent_id === tree.get(['selectedFeedback', 'id'])) { // Patch feedback if it is currently selected
      tree.set(['selectedFeedback', 'comments', {id: data.id}], updatedFeedback);
    }
  } else {
    tree.merge(['ProductPage', 'feedbackData', getTypePlural(type), {id: data.id}], updatedFeedback);
    if (feedback.id === tree.get(['selectedFeedback', 'id'])) { // Patch feedback if it is currently selected
...
export async function addVote(data) {
  ...
  } else { // push on if needs to be created for products
...
export async function addComment({wbpid, id, comment, feedbackId, username}) {
  ...
  await addCommentFeedback({
    wbpid,
    id,
    username: username || (email && email.indexOf('shadow.wikibuy.com') === -1 ? email.split('@')[0] : 'anonymous'),
    comment,
    userId
  });
...
export async function deleteComment({id, wbpid, parent_id, feedbackId}) {
  ...
  if (feedbackId === tree.get(['selectedFeedback', 'id'])) { // Patch feedback if it is currently selected
    const {feedback} = getFeedbackById(feedbackId);
    tree.set(['selectedFeedback'], feedback);
  }
}
