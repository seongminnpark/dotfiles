...
import loadApp from '../content/utility/loadNotification';
...
function setBestAmazonResult(amazonResults, runId) {
  ...
  const results = _(amazonResults)
    ...
    .filter((r) => !!r.savings)
    .sortBy(['savings', (r) => moment(_.get(r, 'details.arrival_date'), 'ddd. MMM. D').unix()], ['desc', 'asc'])
    .reverse()
    .value();
...
export function visitMerchant({details, cartPricing, run, title, meta}) {
  ...
  let paymentsSupport = hasFeature('affiliate_only') ? false : !!_.get(cartPricing, 'paymentsSupport');
  ...
  if (_.get(details, 'product.vendor') === 'amazon.com') {
    sendMetric('trackClick', 'visitAmazon', '', _.assign({
      view: 'productDetailModal',
      domain: location.hostname.replace(/^www\./, ''),
      pagePath: location.pathname,
      pageLocation: 'modal',
    ...
    sendMetric('trackClick', 'visitMerchant', 'check out on', _.assign({
      view: 'productDetailModal',
      domain: location.hostname.replace(/^www\./, ''),
      pagePath: location.pathname,
      pageLocation: 'modal',
  ...
  if (details.product.url && details.product.url.indexOf('mock_click_id') === -1) {
    details.redirectUrl = `https://api.wikibuy.com/v1/redirect?r=true&url=${encodeURIComponent(details.product.url)}&clickId=mock_click_id`.replace('mock_click_id', clickId);
  }

  // replace mock_click_id in atcURL with clickId
  _.set(details, 'details.atcURL', _.get(details, 'details.atcURL', '').replace('mock_click_id', clickId));
  return checkout({paypal: title === 'paypal', detail: details, runId: run.runId || run.id, clickId, priceCheckId: window.priceCheckId, paymentsSupport});
}
