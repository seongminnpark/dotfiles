...
import InfoIcon from 'wikibuy-shared-info-icon';
      ...
      <div className={`wb-CouponListItem ${this.state.copied ? 'copied' : ''}`}>
        <div className="item-container" onClick={this.onClickCopy.bind(this)}>
          <div className="details">
            <span><h4 className="code">{code}</h4>
            {
              restrictions ? <span className="info-icon"><InfoIcon onClick={this.toggleTerms.bind(this)}/></span> : null
            }
            </span>
          </div>
          <div className="copied">
            {
              this.state.copied ?
              <h4 className="copied-text">copied</h4> : null
            }
          </div>
        </div>
        {
          restrictions && this.state.showRestrictions ?
          <h5 className="restrictions">{restrictions}</h5> : null
        }
      </div>
    );
  }
