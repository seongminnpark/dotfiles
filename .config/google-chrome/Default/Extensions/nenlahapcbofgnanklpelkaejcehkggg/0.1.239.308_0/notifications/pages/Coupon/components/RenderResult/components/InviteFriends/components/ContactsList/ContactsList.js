...
function transformContacts(contacts, query) {
  f.set(contacts);
  if (query && query.length < 32) { // 32 is fuse max query length
...
class ContactsList extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (_.isEqual(_.get(nextProps, 'googleContacts.contact'), _.get(this.props, 'googleContacts.contact'))) {
      ...
      <div className='contact-list'>
        <div className={`search ${this.props.hideFooter ? 'overlay' : ''}`}>
          ...
          <input
            ...
            type="text"
            ref={(r) => { if (r) { this.input = r } }}/>
          {
            this.props.hideFooter ?
            <div className="flex">
              <input type="checkbox" ref={(r) => {if (r) { this.selectAll = r}}} className="send-invite" id={`select-all`} onChange={this.onClickSelectAll.bind(this)} />
              <label htmlFor={`select-all`}>Select all</label>
            </div> : null
          }
        </div>
        <ul onMouseLeave={this.setActive.bind(this, -1)} ref='contactScrollList' className="contacts">
          {
            _.map(contacts, (contact, i) => {
              const active = this.state.active === i;
              return (
                <li key={`contact-${i}`} ref={active ? 'activeContactItem' : null}
                    onMouseOver={this.setActive.bind(this, i)}
                    className={active ? 'active' : ''}
                    key={'contact-' + i}>
                    <div className="flex space">
                      <input disabled={contact.invited} type="checkbox" className="send-invite" id={`send-invite-${i}`} checked={_.find(this.props.addedToInvite, {email: contact.email})} onChange={this.props.onAddContact.bind(this, contact)} />
                      <label htmlFor={`send-invite-${i}`}>
                        <div className="flex space">
                          <h4 className="one bold">{contact.name || contact.email}</h4>
                          <h5 className="two">{contact.email}</h5>
                        </div>
                      </label>
                    </div>
                </li>
        ...
        </ul>
        {
          ...
          <footer className="flex space">
            <div className="flex">
              <input type="checkbox" className="send-invite" id={`select-all`} onChange={this.onClickSelectAll.bind(this)} />
              <label htmlFor={`select-all`}>Select all</label>
            </div>
            {
              success ?
              <button className="primary-btn-large sent" disabled><span className="icon-circle-check"></span>Sent</button> :
              awaitingSave ?
              <button className={`primary-btn-large`} disabled={true}>
                <Loader light={true} scale={0.5}>Sending</Loader>
              </button> :
              <button onClick={this.onClickInvites.bind(this)}className="primary-btn-large" disabled={numberAdded === 0}>Send {numberAdded > 0 ? numberAdded : ''} {numberAdded === 1 ? 'invite' : 'Invites'}</button>
            }
          </footer>
        }
        {
          error ?
          <h3 className="error">There was an error, please try again</h3> : null
        }
      </div>
    );
  }
