...
import Dotdotdot from 'wb-geordi-dot-dot-dot';
import ProductImage from 'wikibuy-shared-product-image';
import FiveStarRating from 'wikibuy-shared-five-star-rating';
...
class ProductOffer extends Component {
  ...
  render() {
    ...
    let originShipping = originResult.pricing.shipping ? formatCurrency(originResult.pricing.shipping) : 'Free';
    let originTax = originResult.pricing.tax ? formatCurrency(originResult.pricing.tax) : 'No Tax';

    let bestShipping = _.get(bestResult, 'pricing.shipping') ? formatCurrency(bestResult.pricing.shipping) : 'Free';
    let bestTax = _.get(bestResult, 'pricing.tax') ? formatCurrency(bestResult.pricing.tax) : 'No Tax';
        ...
        </h2>
        {
          deal ?
           <div className="product-wrapper">
            <ProductImage image={image} fadeIn={false}/>
            <div className="offer-content">
              ...
              {
                this.props.rating ?
                <FiveStarRating count={this.props.ratingCount} rating={this.props.rating}/> : null
              }
            </div>
          </div> : null
        }
          ...
          bestResult && savings ?
            <div className="offer-wrapper">
              <div className="breakdown-wrapper">
                ...
                <div className="buttons-wrapper" onClick={this.props.onVisitMerchant.bind(this, bestResult)}>
                  <button className="primary-btn-large full-button">{`Save ${formatCurrency(savings, {removeCents: savings >= 100})}`}</button>
                </div>
                <h5 className="primary-text-link palmetto" onClick={(e) => this.props.onVisitMerchant(bestResult, e)}>View Price History</h5>
              </div>
            </div> :
            <div>
              <button className="primary-btn-large full-button" onClick={(e) => this.props.onVisitMerchant(bestResult, e)}>View Price History</button>
            </div>
        }
      </div>
    );
