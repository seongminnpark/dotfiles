...
class ProductPageAnnotation extends Component {
  ...
  render() {
    ...
    result = result && result.origin ? null : result;
    ...
    const savingsButtonClassName = `${complete ? `${savings || reward ? 'savings' : 'no-savings'}` : 'searching'}`;
  ...
  viewProductPage(label) {
    ...
    if (productPath && runId) {
      window.open(`${WIKIBUY_URL}${productPath}${runId ? `?run=${runId}` : ''}`);
    }
  }
