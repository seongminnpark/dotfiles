...
linus.config.LOGGING_URL = __ENV__ === 'prod' ? 'https://client-logger-api.ivaws.com/log' : 'https://client-logger-api.dev.ivaws.com/log';
...
dewey.emitter.on('result', result => {
  if (result.callSource === 'internal') {
    if (_.get(result, 'pageType')) {
      linus.logDewey(result);
    }
  }
});