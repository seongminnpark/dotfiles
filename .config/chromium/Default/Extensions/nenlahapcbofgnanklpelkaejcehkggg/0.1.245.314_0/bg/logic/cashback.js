...
export async function activateInPinnedTab({url, tabId, pageViewId, domain, affiliateUrl, clickId}) {
  ...
  const result = await runPinnedTab({
    url: affiliateUrl || `${WIKIBUY_API}/redirect?r=1&url=${encodeURIComponent(`http://${domain}`)}&channel=cashback&clickId=${clickId}`,
    id: pinId,
