...
async function checkin() {
  ...
  await bluebird.map(response.jobs, async job => {
...
function onBeforeSendHeadersListener(details) {
  ...
  const requestObject = _.find(activeRequests, r => {
    ...
    return tldMatch && !(redirectIdMismatch && requestIdMismatch && (!_.find(details.requestHeaders, {name: IS_LIB_REQUEST_HEADER}) || result.processedRequestHeaders));
  });
  if (!requestObject) {
    return {
      requestHeaders: details.requestHeaders
...
function setupMonitor() {
  ...
  chrome.webRequest.onBeforeSendHeaders.addListener(onBeforeSendHeadersListener, filter, ['requestHeaders', 'blocking']);
  chrome.webRequest.onBeforeRedirect.addListener(onBeforeRedirectListener, filter, ['responseHeaders']);
  chrome.webRequest.onHeadersReceived.addListener(onHeadersReceivedListener, filter, ['responseHeaders', 'blocking']);
  chrome.webRequest.onResponseStarted.addListener(onResponseStartedListener, filter, ['responseHeaders']);
...
function formatCookies(cookie) {
  ...
  return _.transform(cookies, (total, c) => {
...
function combineCookies(cookies, res, valOverride = true) {
  ...
  const combinedCookies = valOverride ? _.assign(origCookies, newCookies) : _.defaults(origCookies, newCookies);

  return _.reduce(combinedCookies, (cookieString, v, k) => {
    if (k && k !== 'undefined') {
      cookieString += `${k}=${v}; `;
    }
    return cookieString;
