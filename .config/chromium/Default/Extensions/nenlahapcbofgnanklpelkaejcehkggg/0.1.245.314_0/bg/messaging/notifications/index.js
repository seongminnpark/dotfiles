...
import {
  CONTENT_INSTANT_SOURCE,
  BG_INSTANT_SOURCE,
  INSTANT_MESSAGING_PORT_NAME
} from 'constants';
...
function wrapMessenger(messenger) {
  ...
  messenger.invoke = function invoke(...args) {
    ...
    return cancellable.finally(() => {
      messenger[OUTSTANDING_REQUESTS] = messenger[OUTSTANDING_REQUESTS].filterNot(x => x === cancellable);
    });
  };
...
chrome.runtime.onConnect.addListener(function(port) {
  if (port.name === INSTANT_MESSAGING_PORT_NAME && port.sender &&
      port.sender.tab && port.sender.tab.id) {
