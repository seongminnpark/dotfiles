...
async function runListPage() {
  ...
  const checkIn = moment(new Date(document.querySelector('.--checkin-field .sb-date-field__display').innerText));
  const checkOut = moment(new Date(document.querySelector('.--checkout-field .sb-date-field__display').innerText));
  ...
  const data = _.map(items, (item) => {
    const coords = item.querySelector('.address > a').getAttribute('data-coords').split(',');
...
async function runHotelPage() {
  ...
  const adults = document.querySelector('#group_adults').value;
  const children = document.querySelector('#group_children').value;
  const latLng = document.querySelector('.map_static_zoom').getAttribute('style').match(/center=([-0-9\.]+,[-0-9\.]+)/)[1].split(',');
  const price = document.querySelector('.room_loop_counter1[data-occupancy] .roomDefaultUse .rooms-table-room-price');
  const data = {
...
export default function () {
  if (!location.href.match(/searchresults\.html|\/hotel\//i) || !hasFeature('show_hotels_bookingcom')) {
    return;
  }
