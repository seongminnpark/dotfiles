...
function simulateKeyboardEvent(type, el, key) {
  return new Promise(resolve => {
    ...
    if (event.initKeyboardEvent) {
      event.initKeyboardEvent(type, true, true, document.defaultView, false, false, false, false, charCode, charCode);
    } else {
      event.initKeyEvent(type, true, true, document.defaultView, false, false, false, false, charCode, 0);
    }
  ...
  return new Promise(resolve => {
    setTimeout(function() {
      simulateKeyboardEvent('keydown', el, key)
        ...
        .then(() => simulateCustomEvent('input', el))
