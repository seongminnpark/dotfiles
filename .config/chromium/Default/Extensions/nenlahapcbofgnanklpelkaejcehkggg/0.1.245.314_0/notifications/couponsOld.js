...
import Coupon from 'pages/CouponOld/Coupon';
...
const CLASSIFIER_SITES = new Set([
  'etsy.com'
]);
...
async function init() {
  ...
  if (domain === 'groupon.com') {
    sendMetric('track', 'pushCouponScriptEnd', {pageViewId: tree.get('pageViewId')});
  }
  // end TODO
  couponChecking(domain);

  // set browser action just incase the initCoupons promise never resolves
  if (!(tree.get(['couponsDisabledSites', `${currentDomain()}_${tree.get('tabId')}`]) > moment().unix())) {
    if (cashback && !cashback.postCoupons) {
  ...
  let ogPageData;
  try { ogPageData = JSON.parse(sessionStorage.getItem('ogPageData')); } catch (e) {}

  let classifierFired = false;
  dewey.emitter.on('result', async result => {
    if (result.callSource === 'coupons_start' && result.pageData) {
      sessionStorage.setItem('ogPageData', JSON.stringify(result.pageData));
      ...
      couponResult.savings = (ogCartTotal && newCartTotal) ? (ogCartTotal - newCartTotal) : couponResult.savings;
...
async function prepareCouponsApp(coupons, cashback) {
  ...
  const showCoupons = _.has(settings, 'notificationPreferences.showCouponCodes') ? _.get(settings, 'notificationPreferences.showCouponCodes') : true;
  ...
  const loadOptions = {
    ...
    cssUrl: 'GENERATED/couponsOld.css',
    route: (
...
async function getInitCoupons(siteAPIData) {
  ...
  if (_.get(siteAPIData, 'siteData.coupons.type') === 'eeyore' && _.get(siteAPIData, 'siteData.coupons.coupons.length') > 0) {
    ...
    if (identifiers) {
      ...
      const result = dewey.evaluateIdentifierTree(identifiers, deweyDoc);
      if (result) {
        return initOnlyShowCoupons;
      }
    }
    return _.noop;
  } else if (_.get(siteAPIData, 'siteData.coupons.type') === 'tigger' && (shopifyDomain || isTiggerSite(domain))) {
    if (shopifyDomain) {
      ...
      await siteCache(_.assign(siteAPIData, {
