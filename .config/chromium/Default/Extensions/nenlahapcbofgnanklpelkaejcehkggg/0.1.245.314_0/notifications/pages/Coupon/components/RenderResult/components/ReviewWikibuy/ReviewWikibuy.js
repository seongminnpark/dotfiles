  ...
  render() {
    return (
      ...
      <div className="review-wrapper">
        <h4 className="bold charcoal" style={{textAlign: 'center', marginBottom: 15}}>How would you rate Wikibuy?</h4>
        <div className="review-wikibuy">
            {
              _.map(new Array(5), (it, i) => {
                return (
                  <Star onClick={this.starWikibuy.bind(this, i)} rating={i} key={i} type={this.state.deduction <= i ? 'fill' : 'empty'}/>
                );
              })
            }
          </div>
  ...
  rateWikibuy(label, deduction) {
    ...
    if (deduction === 0) {
      if (hasFeature('rw_control')) {
        window.open('https://chrome.google.com/webstore/detail/wikibuy/nenlahapcbofgnanklpelkaejcehkggg/reviews?hl=en-US', '_blank');
