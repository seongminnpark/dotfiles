...
class PriceHistoryDetails extends Component {
  ...
  render() {
    ...
    const changesInLast30 = _(_.get(priceHistory, 'items'))
      ...
      .reduce((accum, item) => {
        if (
          ...
          (_.get(accum, 'previousPrices.match_total_p50') !== item.match_total_p50 || _.get(accum, 'previousPrices.origin_total_p50') !== item.origin_total_p50)
        ) {
          accum.changes += 1;
        }
        accum.previousPrices = {
    ...
    return (
      <AnnotationTooltip onCloseTooltip={this.props.onCloseTooltip} className="price-history-details-tooltip">
        {
          !priceHistory ?
          ...
          <div>
            ...
            <h5>Currently this is the best deal of all Amazon sellers. However, Wikibuy has detected {changes === 1 ? `${changes} price change` : `${changes} price changes`} in the last 30 days.</h5>
            <button className="button-style primary-btn-large" onClick={() => this.props.viewProductPage()}>View price history</button>
          </div> :
          <div>
            <div className="svg-graphic">
              <BestDealSVG/>
            ...
            <h5>Currently this is the best deal of all Amazon sellers. Wikibuy didn’t detect any changes in the last 30 days.</h5>
            <button className="button-style primary-btn-large" onClick={() => this.props.viewProductPage()}>View alternative products</button>
          </div>
        }
      </AnnotationTooltip>
    );
