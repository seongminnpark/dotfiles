      ...
      <div className={this.state.copied ? 'copied' : ''}>
        <div className="coupon-list-item-container" onClick={this.onClickCopy.bind(this)}>
          ...
          <div className="savings">
            {
              this.state.copied ?
              <span className="copied-text">Copied</span> : null
            }
            {
              coupon.success && !this.props.disableChecks ?
              <svg viewBox="0 0 150 150">
                <path d="M10,30 l30,50 l95,-70" fill="none" strokeWidth="15" style={{stroke: '#01c049', strokeDasharray: '180px, 180px', strokeDashoffset: '0px', animationDelay: stagger ? '0ms' : `${this.props.stagger * this.props.index}ms`}} />
              </svg> : null
            }
          </div>
        </div>
      ...
      <div className={`code-list ${props.className || ''}`}>
        {
          _.map(props.coupons, (coupon, i) => {
            return <CouponListItem running={props.running} result={props.result} key={i} index={i} coupon={coupon} stagger={props.stagger} disableChecks={props.disableChecks}/>;
          })
        }
      </div>
    );
