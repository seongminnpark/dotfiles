...
class OptionSelectInput extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="option-select-wrapper">
        <div className={this.state.focus ? 'focus option-select-input' : 'option-select-input'} onClick={this.focusFieldInput.bind(this)} style={styles}>
        ...
        </div>
        {
          ...
          <Autocomplete
            ...
            data={this.props.autoCompleteData}/> : null
        }
      </div>
    );
  }
  ...
  renderInput(focus) {
    ...
    return (
      <div key={'render-input'} className={`select-input ${!items ? 'placeholder' : ''}`}>
        <AutosizeInput
          placeholder={!items ? newInviteFriends ? 'Enter email addresses to see how much Wikibuy credit you can earn' : 'Enter recipient email addresses' : ''}
  ...
  renderSelectItems() {
    ...
    return _.map(this.props.inputItems, (item, i) => {
      ...
      return (
        ...
        <div key={`${i}-${value}`} className={`select-item ${valid}`} onDoubleClick={this.editValue.bind(this, item)} onClick={this.stopProp.bind(this)}>
          <div className="flex-wrapper">
            <span className="value">{item.value}</span>
            <span onClick={this.removeValue.bind(this, item)} className="close"><span className="icon-x"></span></span>
          </div>
        </div>
      );
    });
  ...
  onKeyDown(e, blur) {
    ...
    if (this.state.focus && (e.which === 8) && (!value || value.length === 1) && options && options.length) {
      if (editable && options.length === 1) { // Editing one item left
        e.preventDefault();
        this.removeValue(options[options.length - 1]);
        setTimeout(() => {
          this.newInputField();
        });
      } else if (editable && options.length > 1) { // Editing multple items left
        e.preventDefault();
