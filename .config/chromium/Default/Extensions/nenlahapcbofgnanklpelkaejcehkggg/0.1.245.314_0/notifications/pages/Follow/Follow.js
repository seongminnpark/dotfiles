...
class Follow extends Component {
  ...
  renderReasons() {
      ...
      reasonDivs = _.map(reasons, (reason, i) => {
        ...
        return (
          <div className="reason" key={_.kebabCase(reason)}>
            <input type="checkbox" checked={checked} onChange={onChange.bind(this, reason)} id={_.kebabCase(reason)}/>
            <label htmlFor={_.kebabCase(reason)}><h5 className="text">{reason}</h5></label>
          </div>
        );
      });
    }
    return (
      <div className="reasons-wrapper">
        {reasonDivs}
        <textarea placeholder="tell us more" value={this.state.reasonText} onChange={onTextUpdate.bind(this)}></textarea>
        <button className="primary-btn-small" onClick={this.onSubmit.bind(this)}>Submit</button>
      </div>
    );
  }
  ...
  render() {
    ...
    if (this.state.showReasons || this.state.showComment) {
      ...
      content = (
        <div className="thanks-wrapper"></div>
      );
    } else {
      content = (
        <div className="button-wrapper">
          <button className="secondary-btn-small" onClick={this.onYes.bind(this)}>Yes</button>
          <button className="secondary-btn-small" onClick={this.onNo.bind(this)}>No</button>
        </div>
      );
    }
    return (
  ...
  onYes() {
    ...
    this.setState({
      ...
      showComment: true,
      title: 'Great!',
      subtitle: 'Would you like to leave any additional feedback for future wikibuyers?'
    });
    const metricData = {
      match: true,
      quoteId: data.run.id,
