...
class CustomNotification extends Component {
  constructor(...args) {
    super(...args);
    let display;
    const displaySequence = _.get(_.find(this.props.customNotificationData, 'displaySequence'), 'displaySequence', []);
    ...
    if (this.state.cta) {
      content = (
        <div className="button-wrapper">
          <button className="primary-btn-large" onClick={this.onClick.bind(this)}>{this.state.cta}</button>
        </div>
      );
    }
          ...
          {({opacity, y}) =>
            <div
              ...
              <section>
                ...
                {
                  this.state.subtitle ?
                    <h4 className="bold">{this.state.subtitle}</h4> :
                    null
                }
                {content}
              </section>
  ...
  onClick() {
    ...
    if (this.state.notification < this.state.displaySequence.length - 1) {
      ...
      const url = `https://api.wikibuy.com/v1/redirect?r=true&url=${encodeURIComponent(this.props.customNotificationData.url)}&clickId=mock_click_id`.replace('mock_click_id', clickId);
