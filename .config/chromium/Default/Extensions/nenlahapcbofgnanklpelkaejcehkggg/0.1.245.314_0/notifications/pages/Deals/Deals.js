...
class Deals extends Component {
  ...
  render() {
    ...
    const {title, discount} = _.get(this.props, 'view.deal', {});
        ...
        <Motion
          ...
          {({opacity, y}) =>
            <div
              ...
              <section>
                <h2>{title}</h2>
                <h4>save {currency(discount, true)} at {_.get(this.props, 'view.queryData.title')}</h4>
                <div className="button-wrapper">
                  <button className="primary-btn-large" onClick={this.onClickDeal.bind(this)}>
                    view on Groupon
                  </button>
                </div>
                <h4 className="bold maybe-later tertiary-link" onClick={this.onClosePopup.bind(this, 'no thanks')}>no thanks</h4>
              </section>
            </div>
          }
        </Motion>
  ...
  async onClickDeal() {
    ...
    const redirect = `${_.get(this.props, 'view.deal.url')}&wid=${encodeURIComponent(location.hostname)}&clickId=${clickId}`;
    visitDeal({redirect});
  }
